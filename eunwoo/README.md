...
- **홀덤에 대해 아는 정도**
  _TODO: 중급이상, gto나 핸드레인지, 베팅 규칙 등 기초적인건 알고 아마추어 대회 나가보는 정도
- **그 지식이 봇 만들기에 어떻게 작용했나**
  _TODO: gto기준으로 작성하라고 진행하였고 결과는 썩 좋지 앟았음

### 시행착오

- **실패한 접근 또는 폐기한 가설** (그 시점에 무엇을 보고 결정했는지)
  _TODO: 점수체계가 변경되어서 1등이 무조건 유리한 플레이가 되었음 해당 부분을 변경하게 되었음 그거 말곤 gto로 밀고 나가긴 함
- **다음에 만든다면 바꿀 점**
  _TODO: 아직 깨달은바를 잘 모르겠어서 
- **의외였던 깨달음**
  _TODO: 어설픈 지식은 오히려 ai 활용에 독이 됨

---

## 7. 회고 검토

본인이 적은 섹션 6 회고와 섹션 1~5 자동 추출 자료 사이의 정합성을 직접 검토해서 작성. 메인 README 표준의 출력 양식에 따라 다음 세 가지를 다룬다 (옵션):

- **회고 ↔ 자료 불일치** — 회고가 자료와 충돌하는 항목
- **회고가 추측으로 남긴 항목 중 자료가 답을 주는 경우** — 단정 가능한 부분을 인용으로
- **회고에서 빠진 큰 사건** — 자동 추출에서 발견됐지만 회고에 없는 굵직한 사건

_TODO: 시스템을 만들고 홀덤 룰을 제일 잘 알던 사람이 최하점을 받음.
