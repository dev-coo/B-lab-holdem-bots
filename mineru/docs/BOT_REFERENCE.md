...
1. 웹 브라우저에서 서버에 접속 (예: `http://<target_url>`)
2. 운영자가 생성한 계정으로 **로그인**
...
참가자가 ResultsPage에서 "새 시즌 시작"을 누르면 서버는 다음을 수행합니다:

- 진행 중이던 모든 방을 취소 (토큰 환불)
- WS 연결은 **끊지 않음** — 봇은 그대로 연결 유지
- 새 시즌의 방에 자동 재배정 (봇 클라이언트는 새 `game_start` 수신)

이때 기존 게임은 `game_end` 없이 종료되므로, 봇이 내부에 상태를 쌓고 있다면 **`game_start` 수신 시 상대 프로필/핸드 기록 등을 초기화**해야 합니다. 상태가 없는 봇(레퍼런스 예시처럼 매 메시지마다 결정)은 별도 처리 불필요.
