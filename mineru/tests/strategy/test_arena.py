...
from holdem_agent.strategy.builtins.arena import ARENA_STRATEGIES, ArenaStrategy
...
ARENA_STRATEGY_SLUGS = [
    "arena-lcb-dominator",
    "arena-pressure-conductor",
    "arena-river-lock",
    "arena-draw-leverager",
    "arena-shortstack-commander",
    "arena-blind-matrix",
    "arena-value-siege",
    "arena-counterpunch-lcb",
    "arena-field-synthesizer",
    "arena-final-table-hydra",
]
...
def test_arena_strategy_slugs_are_registered() -> None:
    registered = list_strategies()

    assert [strategy_cls().name for strategy_cls in ARENA_STRATEGIES] == ARENA_STRATEGY_SLUGS
    for slug, strategy_cls in zip(ARENA_STRATEGY_SLUGS, ARENA_STRATEGIES, strict=True):
...
def test_arena_strategies_support_from_genome() -> None:
    genome = StrategyGenome(bluff_frequency=0.09, exploit_aggression=0.74)

    for strategy_cls in ARENA_STRATEGIES:
        strategy = strategy_cls.from_genome(genome)
        assert isinstance(strategy, ArenaStrategy)
        assert strategy.genome == genome


@pytest.mark.parametrize("slug", ARENA_STRATEGY_SLUGS)
def test_arena_strategies_return_legal_actions(
    slug: str,
...
def test_arena_lcb_dominator_keeps_high_signal_edges(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    strategy = get_strategy("arena-lcb-dominator")
    ...
    assert strategy.decide(priced_draw).action == "call"
