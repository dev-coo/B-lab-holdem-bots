...
from holdem_agent.strategy.builtins.practical import PRACTICAL_STRATEGIES, PracticalStrategy
...
def test_practical_strategy_slugs_are_documented_and_registered() -> None:
    registered = list_strategies()

    assert [strategy_cls().name for strategy_cls in PRACTICAL_STRATEGIES] == PRACTICAL_STRATEGY_SLUGS
    for slug, strategy_cls in zip(PRACTICAL_STRATEGY_SLUGS, PRACTICAL_STRATEGIES, strict=True):
...
def test_practical_strategies_support_from_genome() -> None:
    genome = StrategyGenome(bluff_frequency=0.21, exploit_aggression=0.73)

    for strategy_cls in PRACTICAL_STRATEGIES:
        strategy = strategy_cls.from_genome(genome)
        assert isinstance(strategy, PracticalStrategy)
        assert strategy.genome == genome


@pytest.mark.parametrize("slug", PRACTICAL_STRATEGY_SLUGS)
def test_practical_strategies_return_legal_actions(
    slug: str,
